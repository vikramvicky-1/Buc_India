...
import { AlertCircle, Handshake, Shield, CheckCircle, Users, Calendar, ArrowRight, XCircle } from "lucide-react";
...
const Clubs = () => {
  ...
  return (
    <Box component="section" id="clubs" sx={{ position: "relative", pt: { xs: 8, md: 12 }, pb: 12, bgcolor: "#020617", overflow: "hidden" }}>
      {/* Background Decor */}
      <Box sx={{ position: "absolute", top: '20%', right: '-10%', width: '40%', height: '40%', bgcolor: "radial-gradient(circle, rgba(59, 130, 246, 0.03) 0%, transparent 70%)", zIndex: 0 }} />

      {/* Confirm Member Exclusion Dialog */}
      <Dialog open={showSingleClubPopup} onClose={() => setShowSingleClubPopup(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions sx={{ pb: 3, px: 3 }}>
          <Button variant="contained" onClick={() => setShowSingleClubPopup(false)} sx={{ borderRadius: 2, fontWeight: 800 }}>
            Got it
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Container maxWidth="lg" sx={{ position: "relative", zIndex: 10 }}>
        {/* Header Section */}
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'center', md: 'flex-end' }, mb: 8, gap: 4 }}>
          <Box sx={{ textAlign: { xs: 'center', md: 'left' } }}>
            ...
            <Typography variant="h2" sx={{
              ...
              Partner <Box component="span" sx={{ color: "primary.main" }}>Clubs</Box>
            </Typography>
            <Typography variant="body1" sx={{ color: "text.secondary", maxWidth: 500, lineHeight: 1.8, fontWeight: 500 }}>
              Join BUC-approved riding communities across India.
              One club per rider, many adventures together.
            </Typography>
          </Box>
        ...
        {loading ? (
          ...
          <Box sx={{ textAlign: "center", py: 12, bgcolor: 'rgba(255,255,255,0.02)', borderRadius: 6, border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              return (
                <Grid size={{ xs: 12, md: 6, lg: 4 }} key={club.id}>
                  <Card sx={{
                    ...
                    <Box sx={{ position: 'relative', pt: '56.25%', bgcolor: 'rgba(255,255,255,0.03)' }}>
                      {club.logoUrl ? (
                        <CardMedia
                          component="img"
                          image={club.logoUrl}
                        ...
                        <Box sx={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem', fontWeight: 900, color: 'rgba(255,255,255,0.1)' }}>
                          {club.name.charAt(0)}
                        </Box>
                      )}
                      {isMyClub && (
                        <Chip
                          ...
                          label="Your Club"
                          sx={{ position: 'absolute', top: 15, right: 15, bgcolor: 'primary.main', color: 'white', fontWeight: 800, fontSize: '0.65rem' }}
                        />
                      )}
                    </Box>

                    <CardContent sx={{ flexGrow: 1, p: 3.5 }}>
                      <Typography variant="h6" sx={{ color: 'text.primary', fontWeight: 900, mb: 1, lineHeight: 1.2 }}>
                        {club.name}
                      </Typography>
                      {club.moto && (
                      ...
                      <Box sx={{ display: 'flex', gap: 3, mb: 3 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>
                            SINCE {club.startedOn ? new Date(club.startedOn).getFullYear() : 'N/A'}
                          </Typography>
                        </Box>
                      </Box>
                    </CardContent>
                    ...
                    <CardActions sx={{ p: 3, pt: 0, gap: 1.5 }}>
                      {isMyClub ? (
                        <Button variant="outlined" fullWidth disabled sx={{ borderRadius: 2, borderColor: 'rgba(16, 185, 129, 0.4)', color: '#10B981', fontWeight: 800 }}>
                          JOINED
                        </Button>
                      ) : (
                        <Button
                      ...
                      <Button
                        ...
                        sx={{ borderRadius: 2, fontWeight: 800, borderColor: 'rgba(255,255,255,0.1)', color: 'text.secondary' }}
                      >
                        Details
                      </Button>
                    </CardActions>
            ...
            <Container maxWidth="lg">
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center', justifyContent: 'space-between', gap: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <Box sx={{ p: 1, borderRadius: 2, bgcolor: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                    <CheckCircle color="#10B981" size={20} />
                  </Box>
                  <Box>
                    <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 800, fontSize: '0.65rem', textTransform: 'uppercase' }}>Current Active Membership</Typography>
                    <Typography variant="h6" sx={{ color: 'text.primary', fontWeight: 900, lineHeight: 1 }}>{membership.clubId.name}</Typography>
                  </Box>
                </Box>

                <Box sx={{ display: 'flex', w: { xs: '100%', sm: 'auto' }, gap: 2, alignItems: 'center' }}>
                  <TextField
                    size="small"
